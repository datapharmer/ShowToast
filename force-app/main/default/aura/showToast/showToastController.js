...
({
    doInit : function(component, event, helper) {
        // Verify that we have access to force:showToast
        if (!$A.get("e.force:showToast")) {
            console.error("force:showToast event is not available. This component may not be in a supported context.");
            return;
        }
    },
    ...
    invoke : function(component, event, helper) {
        try {
            ...
            helper.showToast('error', 'Error', 'Unable to display the toast message', 5000, 'dismissable', 'error');
        }
    }
})
